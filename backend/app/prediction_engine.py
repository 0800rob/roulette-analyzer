...
# ===========================================================================
# STR 2 — Monitor strategy (calc1/calc2/calc3 + numerical_associations)
# ===========================================================================
#
# Idea (from the user's previous bot):
#   Given the last two spins (current, second), compute three "key values":
#     calc1 = (second + current) mod 36, with 0 mapped to 36
#     calc2 = abs(second - current)
#     calc3 = 36 - calc2
#   Then look up each key value in NUMERICAL_ASSOCIATIONS to get a set of
#   numbers to monitor. The final play is the UNION of all three sets.

NUMERICAL_ASSOCIATIONS: dict[int, list[int]] = {
    1: [1, 12, 21],
    2: [2, 20],
    3: [3, 23, 32],
    4: [4, 33],
...
def compute_monitor_strategy(spins: list[int], window: int = 10) -> dict | None:
    """STR 2 — monitor numbers triggered by a "diff = 10" pair in history.

    Trigger:
      Within the LAST `window` spins, scan from the most recent backwards
      looking for two CONSECUTIVE spins (a, b) where |a - b| == 10. The pair
      must have at least one spin AFTER it (c) to act as `current`.

    The calculation then uses:
        second  = b (the second number of the pair)
        current = c (the spin right after the pair)

    If a triggering pair exists but no spin has come after it yet, returns
    `awaiting_next=True` so the UI can say "aguardando próximo giro".

    Returns None if no diff=10 pair exists at all in the window.
    """
    if not spins or len(spins) < 2:
        return None

    # Slice the rolling window from the end — never look further back.
    recent = spins[-window:]
    n = len(recent)

    # 1) Look for a pair with diff=10 that ALREADY has a `current` after it.
    #    Scan from the most recent backwards so the freshest valid pair wins.
    #    The pair (a, b) and current (c) cannot include 0 — zeros don't count.
    pair_idx: int | None = None
    awaiting_pair: tuple[int, int] | None = None

    # Walk so the most recent valid pair-with-next is found first.
    # We need pair at indices (i, i+1) and current at i+2, all within `recent`.
    ...
    # 2) If no resolved pair was found, check if the LAST two are a pair
    #    (i.e. the trigger just happened, waiting for the next spin).
    #    Again, zeros invalidate the trigger.
    if pair_idx is None and n >= 2:
        a, b = recent[n - 2], recent[n - 1]
        if 0 not in (a, b) and abs(a - b) == 10:
            awaiting_pair = (a, b)

    if pair_idx is None and awaiting_pair is None:
        return None

    if pair_idx is None:
        # awaiting next spin
        a, b = awaiting_pair  # type: ignore[misc]
        return {
            "triggered": False,
            "awaiting_next": True,
            "pair": [a, b],
    ...
    pair_a = recent[pair_idx]
    pair_b = recent[pair_idx + 1]
    second = pair_b
    current = recent[pair_idx + 2]

    raw_sum = (second + current) % 36
    calc1 = raw_sum if raw_sum != 0 else 36
    calc2 = abs(second - current)
    calc3 = 36 - calc2

    a1 = NUMERICAL_ASSOCIATIONS.get(calc1, [])
    a2 = NUMERICAL_ASSOCIATIONS.get(calc2, [])
    a3 = NUMERICAL_ASSOCIATIONS.get(calc3, [])

    monitored = sorted(set(a1) | set(a2) | set(a3))
